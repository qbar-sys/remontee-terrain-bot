...
app.event("message", async ({ event, client, logger }) => {
  ...
  try {
    // Récupérer les URLs des fichiers (Slack renvoie des URLs privées)
    const photoLinks = event.files
      .map((f) => `<${f.permalink}|${f.name || "Photo"}>`)
      .join("\n");

    // Lire la propriété Photos existante pour l'enrichir si plusieurs envois
    const existingPage = await notion.pages.retrieve({ page_id: notionPageId });
    const existingPhotos =
      existingPage.properties["Photos"]?.rich_text?.map((rt) => rt.text.content).join("\n") || "";
    ...
    await notion.pages.update({
      ...
      properties: {
        Photos: { rich_text: [{ text: { content: newPhotos } }] },
      },
    });
